            ...
            <div class="col-md-12">
                <div class="grid simple">
                    <div class="grid-body no-border" style="min-height: 850px;">
                        <br>
                        <div class="row-fluid ">
                            <form action="{{ action('client\MessageController@send') }}" method="POST"
...
<script>
    ...
    function save(){
        $('form').attr('action', '{{ action("client\MessageController@save") }}');
        $('form').submit();
    }
    function submit(){
        $('form').attr('action', '{{ action("client\MessageController@send") }}');
        $('form').submit();
    }
</script>
@endsection
